...
class Wood(object):
    def __init__(self, fuel_type, gamma, water_content, Hu_dry=0):
        ...
        # self.a_2 = (2 * self.C_w + self.H_w / 2 + 2 * self.S_w - self.O_w) / 2
        # self.b_2 = self.C_w
        # self.c_2 = self.H_w / 2
        # self.d_2 = self.S_w
        # self.e_2 = self.a_2 * 3.76
        # self.f_2 = self.N_w / 2
        M_C = 12.0001
        M_O = 15.999
        M_N = 14.007
        ...
        M_S = 32.08
        #check i
        self.M = self.C/M_C*12.011+self.H/M_H*1.008+self.O/M_O*15.999+self.N/M_N*14.007+self.S/M_S*32.08
        self.a_dry = (2 * self.C/M_C + self.H/M_H / 2 + 2 * self.S/M_S - self.O/M_O) / 2
        self.b_dry = self.C/M_C
        self.g_dry = self.u*self.M/(2*M_H+M_O)
        self.c_dry = self.H/(M_H * 2) + self.g_dry
        self.d_dry = self.S/M_S
        self.e_dry = self.a_dry * 3.76
        self.f_dry = self.N/M_N / 2
    ...
    """ 
    def H_in (self, Tair_in, Rel_humidity, Lambda):
        ...
        return self.Hf_nass() + m_O2*O2.h_abs(Tair_in)/O2_MW +  \
               m_N2*N2.h_abs(Tair_in)/N2_MW + m_H2O_Air * H2O.h_abs(Tair_in)/H2O_MW

    def H_out(self,Tair_in, Tout, Rel_humidity, Lambda): # H_out works only  for Lambda > 1
        ...
        #Exhaust Gas; if the exhaust gas temp is low then should use the other values here
        if Tout < 500:
            N2 = Props.Molecule('N2low') #valid 100 to 500 K
        else: 
            N2 = Props.Molecule('N2high') #valid 500 to 2000 K
        if Tout < 700:
            O2ex = Props.Molecule('O2low') #valid 100 to 700 K
        else: 
            O2ex = Props.Molecule('O2high') #valid 700 to 2000 K
        H2O = Props.Molecule('H2O') # valid 500 to 1700 K
        if Tout < 1200:
        ...
        return m_CO2*CO2.h_abs(Tout)/CO2_MW + m_H2O*H2O.h_abs(Tout)/H2O_MW +  \
               m_O2ex*O2ex.h_abs(Tout)/O2_MW + m_N2*N2.h_abs(Tout)/N2_MW 
        """
    ...
    def SubstFuel_mol(self):  # determining the mole fractions of Substitute Fuel for wet wood
        ...
        # print(sum(y))
        return y  # moles fractions for substitute fuel
    ...
    """
    def Cp_ave_stExhaust(self,t_low,t_high):  # this is calculated for dry wood (need to enter w = 0 for wood)
        T_high = t_low + 273.15
        T_low = t_high + 273.15
        # get properties of gases in stoichiometric exhaust
        if (T_low + T_high)/2 < 500:
            N2 = Props.Molecule('N2low') #valid 100 to 500 K   *** These properties need to be for N2_air!
        else: 
            N2 = Props.Molecule('N2high') #valid 500 to 2000 K

        if (T_low + T_high)/2 < 700:
            O2 = Props.Molecule('O2low') #valid 100 to 700 K
        else: 
            O2 = Props.Molecule('O2high') #valid 1200 to 6000 K
        H2O = Props.Molecule('H2O') # valid 500 to 1700 K
        if (T_low + T_high)/2 < 1200:
            CO2 = Props.Molecule('CO2low') #valid 298 to 1200 K
        else: 
            CO2 = Props.Molecule('CO2high') #valid 1200 to 6000 K
        ...
        h_high = m_CO2*CO2.h_abs(T_high)/CO2.I + m_H2O*H2O.h_abs(T_high)/H2O.I + m_N2*N2.h_abs(T_high)/N2.I
        h_low = m_CO2*CO2.h_abs(T_low)/CO2.I + m_H2O*H2O.h_abs(T_low)/H2O.I + m_N2*N2.h_abs(T_low)/N2.I
        return (h_high -h_low)/(T_high - T_low)*1000/m_total
        """